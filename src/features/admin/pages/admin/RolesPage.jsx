...
const RolesPage = () => {
  ...
  // ── Efecto: Cargar roles desde la API REAL
  useEffect(() => {
    const fetchRoles = async () => {
      try {
        const response = await fetch("https://backend-streen.onrender.com/api/auth/roles");
        const result = await response.json();
        
        if (result.status === "success" && result.data?.roles) {
          const mapped = result.data.roles.map(r => ({
            id: r.id_rol,
            name: r.name,
        ...
        showAlert("No se pudieron cargar los roles del servidor", "error");
      }
    };
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      is_active: Boolean(currentRole.isActive),
    };
    ...
    try {
      ...
      if (currentRole.id) {
        // ACTUALIZAR
        response = await fetch(`https://backend-streen.onrender.com/api/auth/roles/${currentRole.id}`, {
          method: 'PUT',
        ...
        // CREAR
        response = await fetch("https://backend-streen.onrender.com/api/auth/roles", {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload)
        });
      ...
      if (result.status === "success") {
        showAlert(currentRole.id ? "Rol actualizado correctamente" : "Rol creado correctamente");
        // Recargar la lista completa desde el servidor para estar seguros
        window.location.reload(); 
      } else {
        showAlert(result.message || "Error al guardar el rol", "error");
      }
    } catch (error) {
  ...
  const confirmDelete = async () => {
    try {
      const response = await fetch(`https://backend-streen.onrender.com/api/auth/roles/${roleToDelete.id}`, {
        method: 'DELETE'
      });
      const result = await response.json();
      
      if (result.status === "success") {
        showAlert(`Rol eliminado correctamente`, "delete");
        setRoles((prev) => prev.filter((r) => r.id !== roleToDelete.id));
      } else {
        showAlert(result.message || "No se pudo eliminar el rol", "error");
      }
    } catch (error) {
